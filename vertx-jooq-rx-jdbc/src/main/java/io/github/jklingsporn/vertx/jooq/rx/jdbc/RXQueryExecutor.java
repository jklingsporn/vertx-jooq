 ...
 */
public class RXQueryExecutor<R extends UpdatableRecord<R>,P,T> implements QueryExecutor<R,T,Observable<P>,Single<P>,Single<Integer>,Single<T>> {
    ...
    @Override
    public Observable<P> findMany(ResultQuery<R> query) {
        return RXTool.executeBlockingObservable(h -> h.complete(query.fetchInto(daoType)), vertx);
    }
