...
public class RowMappers {
    ...
    public static Function<Row,generated.cf.reactive.regular.vertx.tables.pojos.Something> getSomethingMapper() {
        return row -> {
            ...
            pojo.setSomeenum(java.util.Arrays.stream(generated.cf.reactive.regular.vertx.enums.Someenum.values()).filter(td -> td.getLiteral().equals(row.getString("someEnum"))).findFirst().orElse(null));
            pojo.setSomejsonobject(row.get(io.vertx.core.json.JsonObject.class,row.getColumnIndex("someJsonObject")));
            pojo.setSomecustomjsonobject(generated.cf.reactive.regular.vertx.tables.converters.Converters.IO_GITHUB_JKLINGSPORN_VERTX_JOOQ_GENERATE_CONVERTER_SOMEJSONPOJOCONVERTER_INSTANCE.pgConverter().from(row.get(io.vertx.core.json.JsonObject.class,row.getColumnIndex("someCustomJsonObject"))));
